...
class App {
    ...
    update(deltaTime: DOMHighResTimeStamp) {
        ...
        this.gpuRenderer.submitNodeManager(
            this.nodeManager,
            RenderSyncFlags.NodeRenderPosAndOrder
        )
        ...
        // =========================
        {
            ...
            if (focusedNode !== null) {
                ...
                if (tickSinceFocused > this.linkOpenDuration * 0.1) {
                    ...
                    this.overlayCtx.arc(
                        pos.x, pos.y,
                        (focusedNode.renderRadius + lineWidth * 0.5 + 2) * this.zoom,
                        0 - Math.PI * 0.5,
    ...
    getConnectedNodeColorGenerator(nodeColor: color.Color): () => color.Color {
        const nodeColors = tableNodeColors(this.colorTable)
    ...
    recolorWholeGraph() {
        let nodes: Array<DocNode> = this.nodeManager.nodes.slice()

        nodes.sort((a, b) => {
            return b.mass - a.mass
        })

        const alreadyColored: Array<boolean> = new Array(nodes.length).fill(false)
    ...
    focusOnNode(node: DocNode) {
        ...
        this.gpuRenderer.submitNodeManager(
            this.nodeManager, RenderSyncFlags.NodeRenderPosAndOrder
        )
    }
    ...
    const setupDebugUI = () => {
        ...
        addButton(
            'download color table', async () => {
                ...
                util.saveBlob(
                    new Blob([jsonString], { type: 'application/json' }), 'color-table.table'
                )
            }
        )
        ...
        addSlider(
            ...
            (value) => { app.simParam.bhThreshold = value }
        )
